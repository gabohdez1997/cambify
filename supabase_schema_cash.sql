...
-- This function runs every time a movement is inserted to update the session's total_in, total_out and ending_balance
CREATE OR REPLACE FUNCTION update_cash_session_balance()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.type = 'in' THEN
        UPDATE cash_sessions
        SET total_in = total_in + NEW.amount,
            ending_balance = ending_balance + NEW.amount
        WHERE id = NEW.session_id;
    ELSIF NEW.type = 'out' THEN
        UPDATE cash_sessions
        SET total_out = total_out + NEW.amount,
            ending_balance = ending_balance - NEW.amount
        WHERE id = NEW.session_id;
    END IF;
    RETURN NEW;
